...
namespace hds.auth{
	...
	public class AuthSocket{
		...
		private void HandleClientComm(object client){
			...
			// Send initial packet to trigger client response
			// This is a simple "hello" packet that prompts the client to send AS_GetPublicKeyRequest
			byte[] initPacket = new byte[] { 0x00, 0x00 }; // Empty packet to signal connection ready
			try {
				Output.WriteLine("[AUTH] Sending initial packet to client...");
				clientStream.Write(initPacket, 0, initPacket.Length);
				clientStream.Flush();
			} catch {
				Output.WriteLine("[AUTH] Failed to send initial packet");
			}
